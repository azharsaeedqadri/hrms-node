...
const { getResponse, getUserIDByBearerToken } = require("../utils/valueHelpers");

async function addNewDeduction(req, res) {
     try {
          const { name, description, deduction_type, is_fixed, status, company_id } = req.body;
          ...
          const resp = getResponse(addedDeduction, 201, "Deduction Added Successfully.");
...
async function getDeductionByID(req, res) {
     try {
          ...
          const deduction = await Deduction.findByPk(deductionID, {
               attributes: ["deduction_id", "name", "description", "status"],
          });
...
async function editDeduction(req, res) {
     try {
          ...
          const resp = getResponse(updatedDeduction, 200, "deduction updated successfully");
